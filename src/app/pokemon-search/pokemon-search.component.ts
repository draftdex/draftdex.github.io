...
export class PokemonSearchComponent implements OnInit {
    ...
    dbClient: SupabaseClient = createClient(
        'https://kqyshvlibkoatazqavuc.supabase.co',
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlhdCI6MTY0Mzc0MTI3MSwiZXhwIjoxOTU5MzE3MjcxfQ.hMsQnDsKARs4OyTsIpUR2nPR86TQxbvn3hOoyuGEnA8'
    );
    ...
    private getPokemonForTeam(teamName: string | undefined): void {
        ...
        this.supabaseService.getPokemonForTeam(teamName).subscribe({
            next: (pokemon) => {
                let restricteds: Pokemon[];
                let lowerTiers: Pokemon[];
                // Move restricteds to the top of the list
                [restricteds, lowerTiers] = pokemon.reduce(
                    (acc: Pokemon[][], pkmn: Pokemon) => {
                        if (pkmn.tier === 'Restricted') {
                            acc[0].push(pkmn);
                        } else {
                            acc[1].push(pkmn);
                        }
                ...
                // Sort list first by tier, then by name alphabetically
                this.pkmnList = [
                    ...restricteds.sort((a, b) => {
            ...
            error: (error) =>
                console.error(
                    `Error retrieving pokemon for ${teamName}`,
                    error
                ),
        });
    }
    ...
    processFilters() {
        ...
        if (this.filterVals.ability)
            query.or(
                `ability1.ilike.${this.filterVals.ability},ability2.ilike.${this.filterVals.ability},hiddenAbility.ilike.${this.filterVals.ability}`
            );
        ...
        if (this.filterVals.type1 || this.filterVals.type2) {
            if (this.filterVals.dualTypeExclusive) {
                ...
                query.or(
                    `and(type1.eq.${this.filterVals.type1},type2.eq.${this.filterVals.type2}),and(type1.eq.${this.filterVals.type2},type2.eq.${this.filterVals.type1})`
                );
            } else {
                // Dual-typing optional
                query.or(
                    `type1.eq.${this.filterVals.type1},type1.eq.${this.filterVals.type2},type2.eq.${this.filterVals.type1},type2.eq.${this.filterVals.type2}`
                );
            }
        }
    ...
    setPkmnList(pkmn: any) {
        ...
        this.shortList.forEach((elem: any) => {
            let pmonInShortList: any = this.pkmnList.find(
                ({ name }) => name == elem.name
            );
